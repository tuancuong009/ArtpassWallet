...
class RootViewController: UIViewController , InitializationDelegate,CreateInstanceDelegate,UploadImageDelegate,GetDataDelegate, FacialMatchDelegate,DeleteDelegate,AcuantHGLivenessDelegate,CameraCaptureDelegate,LivenessSetupDelegate,LivenessTestDelegate,LivenessTestResultDelegate, LivenessTestCredentialDelegate{
    ...
    @IBOutlet weak var btnTick1: UIButton!
    @IBOutlet weak var btnTick2: UIButton!
    @IBOutlet weak var btnTick3: UIButton!
    
    @IBOutlet weak var IPLivenessLabel: UILabel!
    @IBOutlet weak var IPLivenessSwitch: UISwitch!
    @IBOutlet weak var scrollPage: UIScrollView!
    var checkTick1 = false
    var checkTick2 = false
    var checkTick3 = false
    ...
    @IBAction func idPassportTapped(_ sender: UIButton) {
        if !checkTick1 || !checkTick2 || !checkTick3
        {
            return
        }
    ...
    @IBAction func doTick1(_ sender: Any) {
        self.checkTick1 = !self.checkTick1
        if checkTick1 {
            self.btnTick1.setImage(UIImage.init(named: "ic_checked"), for: .normal)
        }
        else{
            self.btnTick1.setImage(UIImage.init(named: "ic_check"), for: .normal)
        }
    }
    
    @IBAction func doTick2(_ sender: Any) {
        self.checkTick2 = !self.checkTick2
       if checkTick2 {
           self.btnTick2.setImage(UIImage.init(named: "ic_checked"), for: .normal)
       }
       else{
           self.btnTick2.setImage(UIImage.init(named: "ic_check"), for: .normal)
       }
    }
    
    @IBAction func doTick3(_ sender: Any) {
        self.checkTick3 = !self.checkTick3
       if checkTick3 {
           self.btnTick3.setImage(UIImage.init(named: "ic_checked"), for: .normal)
       }
       else{
           self.btnTick3.setImage(UIImage.init(named: "ic_check"), for: .normal)
       }
    }
    @IBAction func doPolicy(_ sender: Any) {
    ...
    func showResult(data:Array<String>?,front:String?,back:String?,sign:String?,face:String?){
        DispatchQueue.global().async {
            ...
            DispatchQueue.main.async {
                self.hideProgressView()
                let storyBoard = STORYBOARD_MAIN
                let resultViewController = storyBoard.instantiateViewController(withIdentifier: "RegisterInforVC") as! RegisterInforVC
                //let storyBoard = STORYBOARD_MainSDK
                //let resultViewController = storyBoard.instantiateViewController(withIdentifier: "ResultViewController") as! ResultViewController
                APP_DELEGATE.acuantObj = AcuantObj.init()
                if(self.capturedFacialMatchResult != nil){
                    ...
                    APP_DELEGATE.acuantObj.data = dataWithFacialData
                }else{
                    APP_DELEGATE.acuantObj.data = data
                }
                APP_DELEGATE.acuantObj.frontImageUrl = front
                APP_DELEGATE.acuantObj.backImageUrl = back
                APP_DELEGATE.acuantObj.signImageUrl = sign
                APP_DELEGATE.acuantObj.faceImageUrl = face
                APP_DELEGATE.acuantObj.username = Credential.username()
                APP_DELEGATE.acuantObj.password = Credential.password()
    ...
    func showHealthCardResult(data:Array<String>?,front:UIImage?,back:UIImage?){
        ...
        DispatchQueue.main.async {
            ...
            APP_DELEGATE.acuantObj.front = front
            APP_DELEGATE.acuantObj.back = back
            self.navigationController?.pushViewController(resultViewController, animated: true)
//            let storyBoard = STORYBOARD_MAIN
//            let resultViewController = storyBoard.instantiateViewController(withIdentifier: "RegisterStep4VC") as! RegisterStep4VC
//            self.navigationController?.pushViewController(resultViewController, animated: true)
        }
        
    }

    // email, verfied code, address, payment, scan
    // IP Liveness
    ...
    public func setCapturedImage(image:Image, barcodeString:String?){
        ...
        if image.image != nil {
            ...
            DispatchQueue.global().async {
             ...
             self.hideProgressView()
            //APP_DELEGATE.showAlert("image not available")
    ...
    public func confirmImage(image:UIImage,side:CardSide){
        ...
        if(side==CardSide.Front){
            ...
            DispatchQueue.main.async() { () -> Void in
                UserDefaults.standard.setValue(image.jpegData(compressionQuality: 0.7)!, forKey: kImageFront)
                UserDefaults.standard.synchronize()
            }
            ...
            if(isHealthCard){
                ...
                DispatchQueue.main.async() { () -> Void in
                    UserDefaults.standard.setValue(image.jpegData(compressionQuality: 0.7)!, forKey: kImageBack)
                    UserDefaults.standard.synchronize()
                }
...
//
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if APP_DELEGATE.isRedoScan {
            APP_DELEGATE.isRedoScan = false
            self.autoScan()
        }
    }
    ...
    func facialMatchFinished(result: FacialMatchResult?) {
        print("FacialMatchResult --->",result)
        self.isProcessingFacialMatch = false
        if(result?.error == nil){
